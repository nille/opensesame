...
const WAKE_AT = "2026-05-23T09:00:00.000Z";

describe("DynamoMessageReader.snoozeThread (ADR-0029)", () => {
  it("queries ThreadIdGSI and SETs snoozed_until on every row in the thread", async () => {
    ...
    const result = await reader.snoozeThread(
      { thread_id: "<root@example.com>", snoozed_until: WAKE_AT },
      NOW,
    );
  ...
  it("issues REMOVE snoozed_until on every row when unsnoozing (null wake)", async () => {
    const updates: UpdateCommand[] = [];
    const client = makeStubClient(async (cmd) => {
      if (cmd instanceof QueryCommand) {
        return {
  ...
  it("returns updated_count: 0 when ThreadIdGSI yields no rows (empty thread)", async () => {
    ...
    const result = await reader.snoozeThread(
      { thread_id: "<orphan@example.com>", snoozed_until: WAKE_AT },
      NOW,
    );
  ...
  it("caps the GSI query at MAX_STAR_FAN_OUT (200) — bounded write cost", async () => {
    ...
    await reader.snoozeThread(
      { thread_id: "<huge@example.com>", snoozed_until: WAKE_AT },
      NOW,
    );
  ...
  it("tolerates per-row ConditionalCheckFailed (phantom row) but counts the survivors", async () => {
    ...
    expect(result.snoozed_until).toBe(WAKE_AT);
  });
